...
# from KAIST.config import make_cfg
from config.kaist_config import make_cfg
from models.BUFFER import buffer
from utils.SE3 import *
from KAIST.dataloader import get_dataloader
...
if __name__ == '__main__':
    ...
    # pdist list from 1 to 10
    # pdist_list = np.arange(6, 11)
    pdist_list = [10]
        ...
        with torch.no_grad():
            ...
            for i in range(num_batch):
                ...
                if i % 30 == 0:
                    ...
                    ply_path = f"../results_ply/KAIST/"
                    if not os.path.exists(ply_path):
                        os.makedirs(ply_path)
                    
                    before_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/KAIST/{pair_name}_before_matching.ply", before_matching)
                    
                    before_matching_gray = src_gray_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/KAIST/{pair_name}_before_matching_gray.ply", before_matching_gray)
                    
                    src_pcd.transform(trans)
                    gt_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/KAIST/{pair_name}_gt_matching.ply", gt_matching)
                    ...
                    o3d.io.write_point_cloud(f"../results_ply/KAIST/{pair_name}_{result}_pred_matching.ply", pred_matching)
