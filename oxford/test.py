...
from config.oxford_config import make_cfg
    ...
    for pdist in pdist_list:
        ...
        with torch.no_grad():
            ...
            for i in range(num_batch):
                ...
                scene = "_".join(data_source['src_id'].split('/')[-1].rsplit('_', 1)[:-1])
                ...
                if i % 10 == 0:
                    ...
                    ply_path = f"../results_ply/Oxford/"
                    if not os.path.exists(ply_path):
                        os.makedirs(ply_path)
                    
                    before_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/Oxford/{pair_name}_before_matching.ply", before_matching)
                    
                    before_matching_gray = src_gray_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/Oxford/{pair_name}_before_matching_gray.ply", before_matching_gray)
                    
                    src_pcd.transform(trans)
                    gt_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/Oxford/{pair_name}_gt_matching.ply", gt_matching)
                    ...
                    o3d.io.write_point_cloud(f"../results_ply/Oxford/{pair_name}_{result}_pred_matching.ply", pred_matching)
