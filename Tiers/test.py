        ...
        with torch.no_grad():
            ...
            for i in range(num_batch):
                ...
                if i % 10 == 0:
                    ...
                    scene = data_source['src_id'].split('_')[1]
                    src_id = data_source['src_id'].split('_')[-1]
                    tgt_id = data_source['tgt_id'].split('_')[-1]
                    pair_name = f"{scene}_{sensor}_{src_id}_{tgt_id}"
                    ply_path = f"../results_ply/tiers/"
                    if not os.path.exists(ply_path):
                        os.makedirs(ply_path)
                    
                    before_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/tiers/{pair_name}_before_matching.ply", before_matching)
                    
                    before_matching_gray = src_gray_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/tiers/{pair_name}_before_matching_gray.ply", before_matching_gray)
                    
                    src_pcd.transform(trans)
                    gt_matching = src_pcd + tgt_pcd
                    o3d.io.write_point_cloud(f"../results_ply/tiers/{pair_name}_gt_matching.ply", gt_matching)
                    ...
                    o3d.io.write_point_cloud(f"../results_ply/tiers/{pair_name}_{result}_pred_matching.ply", pred_matching)
